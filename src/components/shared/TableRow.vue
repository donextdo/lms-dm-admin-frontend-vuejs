<template>
    <v-card v-if="item.length!==0" :height="laptop ? '75px' : '105px'" width="100%" flat class="card" :class="{ 'tutor' : type === 'tutor', 'student' : type === 'student' }">
      ...
      <div class="main-content d-flex align-center">
          ...
          <div class="meta d-flex flex-column justify-center">
              <h4 class="id">{{ item.id < 9 ?'00'+item.id:item.id < 99 ?'0'+item.id:item.id }}</h4>
      ...
      <div class="grade" v-show="type === 'student'">
          <p v-if="type==='student'" class="column-text">{{ item.grade[0].name }}</p>
      </div>
      
      <div class="subject">
          <p  v-if="type==='tutor'" class="column-text">{{ item.subject.name }}</p>
          <p  v-if="type==='student'" class="column-text">{{ item.subject[0].name }}</p>
      </div>
      ...
      <div class="actions">
          <div class="link d-flex" v-if=" !Ssession " @click="showProfile(item)">
            ...
            <div class="link link-icon" v-if="(userType==1) && !Ssession " @click="showForm(item,type)">
            ...
            <div v-if="type=='student' && (userType==1) " class="link link-icon" @click="shift(item.id,item.status,class_id)">
                <svg v-if="!Ssession && item.status=='blocked' ||Ssession && itemStatus==0" width="16" height="20" viewBox="0 0 16 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M16 10C16 8.897 15.103 8 14 8H13V5C13 2.243 10.757 0 8 0C5.243 0 3 2.243 3 5V8H2C0.897 8 0 8.897 0 10V18C0 19.103 0.897 20 2 20H14C15.103 20 16 19.103 16 18V10ZM5 5C5 3.346 6.346 2 8 2C9.654 2 11 3.346 11 5V8H5V5Z" fill="black"/>
                </svg>
                <svg  v-else-if="!Ssession && item.status=='active' ||Ssession && itemStatus==1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><!--! Font Awesome Pro 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M352 144c0-44.2 35.8-80 80-80s80 35.8 80 80v48c0 17.7 14.3 32 32 32s32-14.3 32-32V144C576 64.5 511.5 0 432 0S288 64.5 288 144v48H64c-35.3 0-64 28.7-64 64V448c0 35.3 28.7 64 64 64H384c35.3 0 64-28.7 64-64V256c0-35.3-28.7-64-64-64H352V144z"/></svg>
            </div>
            <div class="link d-flex" v-if="Ssession " @click="showProfile(item)">
              <span>View Details</span> 
              <svg width="8" height="15" viewBox="0 0 8 15" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-if="(userType==1) && !Ssession  " class="link delete-icon" @click="$emit('delete', item.user.name,item.id)">
  ...
  export default {
      ...
      methods: {
        ...
        async getStatus(){
            ...
            const {data}= await axios.get(this.$hostname+'/api/'+this.user+'/sessionStatus/'+this.class_id+'/'+this.item.id)
