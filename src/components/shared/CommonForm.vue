    ...
    >Something went wrong!</v-alert>
        ...
        <v-card :height="laptop ? '540px' : '770px'" :width="laptop ? '400px' : '620px'" class="d-flex flex-column align-center card" >
          ...
          <div class="rowsingle" v-if="type !== 'class'"  >
            <ModelTextInputVue v-if="type !== 'class'"  :placeholder="'Mention the name of the '+type "
             type="text" :label=" type[0].toUpperCase() + type.slice(1) + ' name'" :modelValue="member.name" @update:modelValue="newValue => member.name = newValue"  
             />
          </div>  
          <div class="rowsingle"  v-if="type !=='student'">
              <div  v-if="type !=='student'" class=" form-control">
                <ModelTextInputVue v-if="subjects" :full="true" :placeholder="'Mention the name of the '+type "
                :initialValue="temporySubject ? temporySubject : subjects.subjects[0]"
                type="select" :items="subjects.subjects" label="Subject taught" :modelValue="member.subject_id" @update:modelValue="newValue => member.subject_id = newValue"/>   
              </div>      
          </div>
          <div class="rows" v-if="type ==='class'">

            <div  v-if="type ==='class'" class=" form-control">
              <ModelTextInputVue v-if="grades"
                :initialValue="temporyGrade ? temporyGrade : grades.grades[0]"
                type="select" :label="'Select Grade'" :items="grades.grades" 
                :modelValue="member.grade_id" @update:modelValue="newValue => member.grade_id = newValue"
                
                />
            </div>
            <div v-if="type ==='class'" class=" form-control">
                <ModelTextInputVue v-if="tutors"
                :initialValue="temporyTutor ? temporyTutor : tutors.tutors[0]"
                type="select" :label="'Select Tutor'" :user="true" :items="tutors.tutors" 
                :modelValue="member.tutor_id" @update:modelValue="newValue => member.tutor_id = newValue"
          ...
          <div class="rows" v-if="type ==='class'">
              <div v-if="type ==='class'" class=" form-control">
                  <ModelTextInputVue v-if="countries"
                  :initialValue="temporyCountry ? temporyCountry : countries.countries[0]"
                  type="select" :label="'Select Country'" :items="countries.countries" 
                  :modelValue="member.country_id" @update:modelValue="newValue => member.country_id = newValue"
                  
                  />
              </div> 
              <div  v-if="type ==='class'" class="  form-control-price">
                <ModelTextInputVue placeholder='Mention the price' 
                  type="text" style="--brd:5px 0px 0px 5px" label=' Price' :modelValue="member.price" @update:modelValue="newValue => member.price = newValue"  
                  />
                   
                    <label style="margin-top:24px;    border-radius:0px 5px 5px 0px; position:relative;" class="label" ><span style="position:relative;background-color:#FFA500;padding-top:1px;padding-bottom:1px;padding-left:0; color:white;top:6px; font-size:16px;">{{currency}}</span> </label>

              </div>
          </div> 
          <div class="rows" v-if="type ==='class'">
              <div class=" form-control" v-if="type === 'class'">
                  <ModelTextInputVue v-if="day_of_week"
              ...
              <div class=" form-control" v-if="type === 'class'">
                  ...
                  type="time" label=" Time" :modelValue="member.time" @update:modelValue="newValue => member.time = newValue"   
                  />
              </div>
          </div>
          <div class="rowsingle" v-if="type !=='class'">
              <div class=" form-control" v-if="type !== 'class'">
                        <ModelTextInputVue placeholder="Email"
                    type="text" label=" Email" :modelValue="member.email" @update:modelValue="newValue => member.email = newValue"
         ...
         <div class="rowsingle" v-if="type !=='class'">      
              <div class=" form-control" v-if="type !== 'class'">
                <ModelTextInputVue placeholder="Contact no"
                type="text" label=" Contact no" :modelValue="member.contact_no" @update:modelValue="newValue => member.contact_no = newValue" 
                />
              </div>
                     </div>
          ...
          <ButtonVue v-else :text="'Add ' +type[0].toUpperCase() + type.slice(1)" :modal="true"   @click="add_member()"/>
        ...
        toLocalTime(){
          ...
          setInfo(){
           if(this.type!=='class') 
           {
            ...
            this.member.contact_no = this.info.user ? this.info.user.contact_no :null
         ...
         setCurrency(){
          this.countries.countries.filter((country)=>{ if(country.id==this.member.country_id ) {this.currency = country.currency}})
         },
           ...
           await axios
                ...
                .then(response => {
                    if (response.status == 200) {
                        ...
                        this.member.tutor_id= this.tutors.tutors[0] ? this.tutors.tutors[0].id:null
